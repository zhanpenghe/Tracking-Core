...
int get_beacon_name_from_url(struct mg_connection *c, char *str)
{
	...
	if(strcmp(part, "tag") == 0)
	{
		...
		if(temp != NULL){
			...
			found_mac = get_beacon_mac_addr(id);
		}
		...
		if(found_mac != NULL){
			...
			//x, y
			pos_offset = get_pos_by_mac(get_pos_list(), found_mac, pos_message, offset);
			...
			if(pos_offset > offset){
				...
				pos_message[offset] = '\"';
				len = (int) strlen(id);
				strncpy(pos_message+offset+1, id,len);
		...
		mg_printf(c,
			...
			(int) strlen(pos_message), pos_message);
		return 1;
	}else if(strcmp(part, "agent_status")==0){
		temp = part;
		temp = strtok(NULL,"/");
		memset(id, 0, 32);
		if(temp != NULL){
			strncpy(id, temp, strlen(temp)+1);
			printf("%s\n", id);
			i = check_status(get_mac_by_id(get_agent_infos(), get_agent_num(), id), get_agent_list());
...
static void ev_handler(struct mg_connection *c, int ev, void *p) {
	...
	if (ev == MG_EV_HTTP_REQUEST) {
		...
		i = get_beacon_name_from_url(c, url);
	}
}
