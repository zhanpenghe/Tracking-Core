...
from pexpect import pxssh                   # for automatic agent implementation
import os
import time
import traceback
...
import alg_enhancement as enhancement
#import alg.ls as ls
...
class BeaconClass:
    ...
    def position(self,a_pos):
        ...
        if a==0:
            ...
            #current_pos = self.apply_enhancement(current_pos,self.cur_room)
            ...
            if b != None:
                self.truepos = self.apply_enhancement(b,self.cur_room)
    ...
    def apply_enhancement(self,pos_in,room_in):
        c_pos = enhancement.add_enhancement(pos_in, room_in)
        return [int(c_pos[0]),int(c_pos[1])]
    # Calculate Average
    def pos_avg(self):
...
def main_local(time_interval=0.0015):
    ...
    # Main Program
    with open("data/D003.Video.txt") as inputLogFile:
